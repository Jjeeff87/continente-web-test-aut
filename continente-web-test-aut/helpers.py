...
def get_env(key, default=None):
    """Reads an environment variable (from .env or the system), with a fallback default value."""
...
def human_type(element, text, min_delay=0.06, max_delay=0.18):
    """Types a text character by character with small random pauses,
    mimicking human typing instead of filling the field instantly
    (send_keys(text) all at once). Reduces the chance of the test's
    behavior looking obviously automated.
    """
